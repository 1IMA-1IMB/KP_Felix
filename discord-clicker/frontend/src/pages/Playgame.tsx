...
const Playgame: React.FC = (): JSX.Element => {
  ...
  useEffect(() => {
    const interval = setInterval(() => {
      if (!loading) {
        ...
        axios
          .post("http://localhost:3000/updategamesave", {
            token,
            guildId,
            money,
          })
    ...
    <div>
      {loading ? (
        ...
        <div className="game" style={{ backgroundColor: `${game.color}` }}>
          <div className="navbar flex justify-between items-center pt-4">
            <a href="/">
              <h1 className="text-white text-2xl sm:text-5xl font-bold italic ml-10">
                {game.name}
              </h1>
            </a>
            <div className="profile flex items-center gap-4 sm:gap-10 mr-10">
          ...
          <div className="content min-h-screen flex flex-col items-center justify-center">
            <h1 className="text-5xl text-white font-bold text-center">
              You have {money} {game.currency}
            </h1>
            <h1
              className="text-8xl mt-10 cursor-pointer select-none"
              onClick={addMoney}
            >
              {game.emoji}
            </h1>
